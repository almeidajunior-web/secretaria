...
// Compras, grouping by due date is always on (it's the central organizing
// concept here), so there's no group-toggle button. While `selectMode` is
// active the whole middle section gives way to a bulk-actions bar.
export default function VencimentosToolbar({
  sortChain,
  onToggleSort,
  filters,
  onToggleFilter,
  ...
  const activeFilterCount = filters.categoryIds.length
  ...
  if (selectMode) {
    return (
      <BulkActionsBar
        ...
        onBulkTogglePaid={onBulkTogglePaid}
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-center gap-3 glass border-b px-4 py-2.5">
      ...
      >
        {filters.hidePaid ? <EyeOff size={12} /> : <Eye size={12} />}
        {filters.hidePaid ? 'Ocultando pagas' : 'Mostrando pagas'}
      </button>
...
function BulkActionsBar({
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border bg-accent-soft/40 px-4 py-2.5">
      ...
      <button
        type="button"
        onClick={() => onBulkTogglePaid(false)}
        disabled={selectedCount === 0}
        className="inline-flex items-center gap-1.5 rounded-md border border-border-strong px-2.5 py-1 text-[11px] font-medium text-text-secondary hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"
      >
        <Circle size={12} />
...
function FilterPopover({ open, onOpenChange, filters, onToggleFilter, onClearFilters, categories, activeFilterCount }) {
